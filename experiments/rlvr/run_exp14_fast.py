"""
exp14 v2 — Causal Trajectory Transfer (fast production run)

Phase 1: Build quadrant dataset using vLLM batch inference (fast)
Phase 2: Activation patching using PyTorch hooks (sequential, but
         only on the examples we need — much smaller set)

Target: ≥50 frontier (D) examples with proper per-example logging.

Usage:
    python run_exp14_fast.py 2>&1 | tee /marimo/exp14_production.log
"""
...
# Layers to patch (individual + cumulative)
...
# How many examples per quadrant to use for patching
TARGET_PATCH_EXAMPLES = 60
# For dataset building: evaluate this many GSM8K examples total
DATASET_EVAL_N = 2000
MAX_NEW_TOKENS_EVAL = 256
MAX_NEW_TOKENS_PATCH = 256
...
# ── Phase 1: build dataset with vLLM ─────────────────────────────────────────
def build_dataset_vllm():
    ...
    log("=== Phase 1: Building quadrant dataset with vLLM ===")
    ...
    sampling = SamplingParams(temperature=0.0, max_tokens=MAX_NEW_TOKENS_EVAL)

    results = {k: [] for k in ("A_Core", "B_SFT_only", "C_Both_fail", "D_Frontier")}

    for model_key in ("SFT", "RLVR"):
        model_id = MODELS[model_key]
        log(f"Running vLLM inference for {model_key} ({model_id}) ...")
        llm = LLM(
            model=model_id,
            dtype="bfloat16",
            max_model_len=4096,
        ...
        log(f"  Generated {len(texts)} outputs")

        # Store in golds
        for i, g in enumerate(golds):
            g[f"{model_key}_text"] = texts[i]
            g[f"{model_key}_correct"] = is_correct(texts[i], g["gold"])
            ...
            try:
                ...
                for l in INDIVIDUAL_LAYERS:
                    ...
                    gen = patched_generate(
                        sft_model, inp, rlvr_cache, [l], MAX_NEW_TOKENS_PATCH, eos_id
                    )
                    text = tokenizer.decode(gen[0], skip_special_tokens=True)
                    res["RLVR_to_SFT"][q_name]["individual"][lkey].append(
                        int(is_correct(text, gold))
                    )
                    # SFT → RLVR
                    gen = patched_generate(
                        rlvr_model, inp, sft_cache, [l], MAX_NEW_TOKENS_PATCH, eos_id
                    )
                    text = tokenizer.decode(gen[0], skip_special_tokens=True)
                ...
                for seg_start, seg_end in CUMULATIVE_SEGMENTS:
                    ...
                    gen = patched_generate(
                        ...
                        rlvr_cache,
                        seg_layers,
                        MAX_NEW_TOKENS_PATCH,
                        eos_id,
                    )
                    text = tokenizer.decode(gen[0], skip_special_tokens=True)
                    hit_r2s = int(is_correct(text, gold))
                    ...
                    text = tokenizer.decode(gen[0], skip_special_tokens=True)
                    hit_s2r = int(is_correct(text, gold))
                    res["SFT_to_RLVR"][q_name]["cumulative"][skey].append(hit_s2r)

                # Verbose per-example log
                r2s_12 = res["RLVR_to_SFT"][q_name]["cumulative"].get("L0-12", [])
                s2r_12 = res["SFT_to_RLVR"][q_name]["cumulative"].get("L0-12", [])
                log(
                    f"  [{q_name}] ex {idx + 1:3d}/{len(subset)}  gold={gold:>8s}  "
                    f"R→S@L0-12={'✓' if r2s_12 and r2s_12[-1] else '✗'}  "
                    f"S→R@L0-12={'✓' if s2r_12 and s2r_12[-1] else '✗'}"
                )
...
def print_summary(res):
    ...
    log("\nRESULTS: Individual Layer — Dataset D")
    for l in INDIVIDUAL_LAYERS:
        key = str(l)
        r2s = (
            res["RLVR_to_SFT"].get("D_Frontier", {}).get("individual", {}).get(key, [])
        )
        s2r = (
            res["SFT_to_RLVR"].get("D_Frontier", {}).get("individual", {}).get(key, [])
        )
        m1, l1, u1 = ci(r2s)
        m2, l2, u2 = ci(s2r)
        log(
...
if __name__ == "__main__":
    log("=== exp14 v2 — Causal Trajectory Transfer ===")
    log(f"Output  : {OUT_PATH}")
    log(f"Dataset : {DATASET_PATH}")

    quadrants = build_dataset_vllm()
    res = run_patching(quadrants)
